...
Conductor's ``claude-agent-sdk`` provider once derived
``ClaudeAgentOptions.add_dirs`` from the directory arguments of every stdio
MCP server, on the stated belief that forwarding them "restores the declared
scope" the CLI would otherwise collapse. It does not, and the reason is a
property of the *server*, not of Conductor:
